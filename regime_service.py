...
try:
    from news_monitor import news_cache
...
META_BLOCK = {
    ...
    "provision_url": "https://api.harrisoncollab.com/v1/keys/provision",
...
def analysis_to_regime_response(analysis_result: dict, symbol: str, timeframe: str,
    ...
    if NEWS_MONITOR_AVAILABLE:
        alert = news_cache.get(symbol.upper())
        if alert.level != "none":
            news_alert = {
                "level": alert.level,
    ...
    # Override action on extreme news alert
    if news_alert and news_alert["level"] == "extreme":
        action = "avoid"
        action_detail = f"NEWS ALERT: {alert.detail}. Structure may not yet reflect event. Avoid trading."
        stability.halflife_minutes = min(stability.halflife_minutes, 15.0)
        stability.urgency = "exit_now"
        stability.instability_risk = "high"
    elif news_alert and news_alert["level"] == "elevated":
        stability.halflife_minutes *= 0.5
        action_detail_suffix = f" NEWS: Elevated activity — {alert.detail}."
    else:
        action_detail_suffix = ""

    # Append urgency context to action_detail
    action_detail = _append_urgency_context(action_detail, stability)
    if not (news_alert and news_alert["level"] == "extreme"):
        action_detail += action_detail_suffix if news_alert and news_alert["level"] == "elevated" else ""

    # Transition warning — premium feature (enterprise only)
    transition_warning = None
    if tier in ("enterprise",):
        try:
            from regime_drift import compute_transition_warning
            tw = compute_transition_warning(
                regime=regime,
                dhs=dhs,
                analysis_result=analysis_result,
                recent_dhs_values=recent_dhs_values or [],
                dominant_sequence=analysis_result.get("temporal", {}).get("dominant_sequence", []),
            )
            if tw.drift_score > 0.15:
                transition_warning = {
                    "drifting_toward": tw.drifting_toward,
                    "drift_score": round(tw.drift_score, 2),
                    "estimated_transition_minutes": round(tw.estimated_transition_minutes) if tw.estimated_transition_minutes else None,
